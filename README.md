...
## 內容
	...
	docker/                        固定版 BlastEm ＋ libvgm 的環境
	  ...
	  md_fight.py                  自動打完一場戰鬥（含「無敵」：把 HP 寫回上限）
...
12. **要判斷「現在卡在哪一層 UI」，先看堆疊指標，不要去 RAM 裡找旗標。**
    ...
    「完美」的假旗標 —— 那只是呼叫深度剛好相同。
13. **先讀原廠說明書再寫自動化。** 那片的戰鬥選單有八個指令，只有一個
    （`Attack`：打最前面那隻）**沒有後續提示**，其餘都會問「打哪一隻」。
    不知道這件事就會盲送確認鍵，掉進子選單再被取消、繞回原地 ——
    實測連按 60 次，回合數還停在 1。掃描器與反組譯都看不出這件事，
    說明書一句話就講完了。
14. **「滿血時目前值等於上限」會讓 HP 的搜尋結果多出假答案。**
    用全隊 HP 的位元組序列掃 RAM，命中的位址不只一個，
    要**挨過打**才分得出哪個是目前值。挑錯了讀起來一切正常
    （永遠是 `16/16`），寫回去等於什麼都沒做。
