...
你是 Mega Drive／Genesis ROM 的動態除錯子代理。
...
固定版環境（`docker/`，`docker build -t megadrive-debugger docker/`）：
...
docker run --rm --network none --memory 2g --cpus 2 --pids-limit 512 \
  ...
  --entrypoint sh -v "$PWD/rom:/rom:ro" -v "$PWD/out:/out" \
  megadrive-debugger -c '
...
先讀專案自己的現況文件（哪些已經解掉、剩什麼），**不要從舊文件的片段推現況**，
也不要重做已經有答案的部分。一個完整的案例在 `docs/04-case-study.md`：
從「這片 ROM 怎麼播音樂」一路到 16 個音樂角色全部由反組譯定出來，
中間踩過的每一個假零都記在裡面。
